...
- [X] T001 Verify no new dependency or secret is required: confirm `discord.js` is already a dependency in `package.json` (its `REST` client is used) and that `DISCORD_BOT_TOKEN` is already present in `.env.example` and the `KEYS=( … )` array in `scripts/set-secrets.sh`. No file change expected; if `DISCORD_BOT_TOKEN` is somehow absent from either, add it (name only).
...
- [X] T014 [P] Update `docs/OPERATIONS.md`: add a "Bot-mode delivery targets" subsection — how to add a `mode='bot'` target (channel_id required, guild_id optional, no webhook_url_ref), the operational precondition (bot must be in the guild with permission to post in the channel), and how a permission/channel error surfaces as a `failed` delivery_log row. Mirror the existing target/source wiring docs. State rules directly; no spec/FR citations (Principle V).
- [ ] T015 Run the `quickstart.md` end-to-end validation against a real channel: bot post (US1), dual-mode fan-out, idempotency, operator-safe failure (US2), repoint-with-no-redeploy. Confirm each success signal maps to its SC. **Status: automated portion done** (the `quickstart.md` "Automated checks" — `check:all` + the `bot-delivery` suite — pass, proving every behavior with fakes). The manual live-channel steps require a logged-in bot, a real DB, and a Discord channel the bot can post in — a deploy/operator step to run after merge, same as the 001/002 live tests. Left unchecked until that live run is performed.
- [X] T016 Run `npm run check:all` (format + lint + typecheck + test) and confirm green on a clean checkout — the release gate (Principle V). Confirm the full 001/002 suite still passes (webhook-path no-regression, SC-006).
