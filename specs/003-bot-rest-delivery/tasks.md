...
- [X] T015 Run the `quickstart.md` end-to-end validation against a real channel: bot post (US1), dual-mode fan-out, idempotency, operator-safe failure (US2), repoint-with-no-redeploy. Confirm each success signal maps to its SC. **Done** — deployed 003 to staging and ran the live e2e against a real Discord channel: bot-mode delivery posted as the bot (`delivery_log` `ok`), dual-mode fan-out (bot + webhook both `ok`), idempotency (posted once under a race), and the operator-safe permission failure (`failed | Unknown Channel`, isolated, inbound acked). All success signals mapped to their SCs.
